...
function isMissingSchemaColumnError(err, table, column) {
  const text = [
    err?.code,
    err?.message,
    err?.details,
  ...
  return (
    ...
    (text.includes(tableName) || text.includes('schema cache') || text.includes('pgrst204')) &&
    (text.includes('schema cache') || text.includes('could not find') || text.includes('column') || text.includes('pgrst204'))
  );
}
...
async function resolveCustomerId(customerName, ownerUserId) {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) {
    if (isMissingSchemaColumnError(error, 'customers', 'user_id')) {
      writeSchemaColumnCache('customers', 'user_id', false);
      ...
      return retry.data?.customer_id || null;
    }
    throw error;
  }
